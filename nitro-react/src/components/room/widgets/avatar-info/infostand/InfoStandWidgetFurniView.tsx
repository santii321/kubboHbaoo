...
export const InfoStandWidgetFurniView: FC<InfoStandWidgetFurniViewProps> = (
  props
) => {
  ...
  useSoundEvent<NowPlayingEvent>(
    ...
    (event) => {
      ...
      const songInfo =
        GetNitroInstance().soundManager.musicController.getSongInfo(event.id);
  ...
  useEffect(() => {
    ...
    if (
      ...
      (avatarInfo.usagePolicy ===
        RoomWidgetFurniInfoUsagePolicyEnum.CONTROLLER &&
        isValidController) ||
      (avatarInfo.extraParam === RoomWidgetEnumItemExtradataParameter.JUKEBOX &&
        isValidController) ||
    ...
    if (avatarInfo.extraParam) {
      if (
        ...
        const playlist =
          GetNitroInstance().soundManager.musicController.getRoomItemPlaylist();
        ...
        avatarInfo.extraParam.indexOf(
          RoomWidgetEnumItemExtradataParameter.SONGDISK
        ) === 0
      ) {
        furniSongId = parseInt(
          avatarInfo.extraParam.substr(
            RoomWidgetEnumItemExtradataParameter.SONGDISK.length
          )
        );
    ...
    if (godMode) {
      ...
      if (roomObject) {
        ...
        const furnitureData = roomObject.model.getValue<{
          [index: string]: string;
    ...
    if (avatarInfo.isOwner || avatarInfo.isAnyRoomController)
      ...
      avatarInfo.isRoomOwner ||
      avatarInfo.roomControllerLevel >= RoomControllerLevel.GUILD_ADMIN
    )
    ...
    if (avatarInfo.groupId)
      SendMessageComposer(
        new GroupInformationComposer(avatarInfo.groupId, false)
      );
  ...
  useEffect(() => {
    const songInfo =
      GetNitroInstance().soundManager.musicController.getSongInfo(songId);
    ...
    (action: string) => {
      ...
      switch (action) {
        ...
        case "save_custom_variables": {
          ...
          SendMessageComposer(
            new SetObjectDataMessageComposer(avatarInfo.id, map)
          );
          break;
        }
      }
  ...
  const getGroupBadgeCode = useCallback(() => {
    ...
    if (!stringDataType || !(stringDataType instanceof StringDataType))
      return null;
  ...
  return (
    <Column gap={1} alignItems="end">
      <Column className="nitro-infostand rounded">
        <Column
        gap={0}
          className="contentenido-area"
          >
          <Column gap={0}>
            <Flex alignItems="center" className='textoJust' justifyContent="between" >
              <Text variant="white" small wrap>
                {avatarInfo.name}
              </Text>
              <FaTimes className="cursor-pointer fa-icon close" onClick={onClose} />
            </Flex>
            <hr className="m-0" />
          </Column>
          <Column gap={0}>
          ...
          <Column gap={1}>
            ...
            {customKeys.length > 0 && (
              ...
              <Flex gap={2} justifyContent="end" className='p-1'>
        {canMove && (
          <button
            className='botonMove'
        ...
        {pickupMode !== PICKUP_MODE_NONE && (
          ...
          className='botonMove'
            ...
            <i className='icon icon-pickup'></i>
          </button>
        )}
        {canUse && (
          <button
