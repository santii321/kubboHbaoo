...
export const RoomToolsWidgetView: FC<{}> = (props) => {
    ...
    switch (action) {
      ...
      case "zoom":
        setIsZoomedIn((prevValue) => {
          let scale = GetRoomEngine().getRoomInstanceRenderingCanvasScale(
            roomSession.roomId,
            1
          );
      ...
      case "navigator_search_tag":
        ...
        SendMessageComposer(
          new NavigatorSearchComposer("hotel_view", `tag:${value}`)
        );
        return;
    }
  };
  ...
  return (
    <Flex className="nitro-room-tools-container">
      <Flex className="nitro-room-tools p-2" gap={1}>
        <Base
          pointer
          title={LocalizeText("room.settings.button.text")}
          className="icon icon-ui_navigator_icon_settings_on positionIcon"
