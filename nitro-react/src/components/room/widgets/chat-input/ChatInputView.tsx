  ...
  const chatStyleIds = useMemo(() => {
    ...
    for (const style of styles) {
      ...
      if (style.isSystemStyle) {
        if (
          GetSessionDataManager().hasSecurity(RoomControllerLevel.MODERATOR)
        ) {
          styleIds.push(style.styleId);
  ...
  return createPortal(
    <>
      <Base
        ...
        className="icon icon-chat_history positionIcon"
      />
      <div className="nitro-chat-input-container">
        ...
        <div className="input-sizer align-items-center w-100">
          {!floodBlocked && (
            <input
              ref={inputRef}
              type="text"
              className="chat-input"
              placeholder={LocalizeText("Escriba aqui")}
              value={chatValue}
...
function setIsZoomedIn(arg0: (prevValue: any) => boolean) {
